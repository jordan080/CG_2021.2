...
TEXnotex *CarregaTextura(char *arquivo, bool mipmap)
{
	...
	int indice = _procuraTexturaNoTex(arquivo);

	if(indice!=-1)
		return _texturasNoTex[indice];

	TEXnotex *pImage = CarregaJPG(arquivo);	
	...
	free(pImage->data); 	
	_texturasNoTex.push_back(pImage);
	return pImage;
}
...
void DecodificaJPG(jpeg_decompress_struct* cinfo, TEXnotex *pImageData, bool inverte)
{
...
TEXnotex *CarregaJPG(const char *filename, bool inverte)
{
	struct jpeg_decompress_struct cinfo;
	TEXnotex *pImageData = NULL;
	FILE *pFile;
	...
	pImageData = (TEXnotex*)malloc(sizeof(TEXnotex));
